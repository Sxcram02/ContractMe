<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if(!isset($userEmailLogin) and !empty($userPassLogin) and $valueOption == "Iniciar Sesion") {    
            getAccount($valueOption, $userEmailLogin,$userPassLogin);
            $_SESSION['user']= $userEmailLogin;
            exit();
        }

        if(empty($userEmailLogin) and empty($userPassLogin) and $valueOption == "Crear cuenta"){
