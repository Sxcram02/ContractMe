    ...
    public function setUser (string $clientName, string $clientUserName, string $clientEmail, string $clientPhone) {
        if(func_get_args() > 2){
            ...
            while (mysqli_affected_rows($databaseConnection) == 0) {
                $queryINSERT = "INSERT INTO users (userName, userAp1, email, telefono) VALUES ('$clientName', '$clientUserName', '$clientEmail', '$clientPhone');";
                $doQueryInsert1 = $databaseConnection -> query()
