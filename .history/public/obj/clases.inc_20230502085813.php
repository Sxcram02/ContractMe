...
class Database {
    ...
    public function setUser (array $userListData) :bool {
        $tester= false;
        if(func_num_args()==1){
            foreach ($userListData as $userData) {
                (empty($userData))?$tester:true;
            }

            $DataInsert = implode(", ",$userData);
            $databaseConnection = $this -> objNewMysql;
            $queryInsert = "INSERT INTO users VALUES ($DataInsert);";
            $resultaQuery = $databaseConnection  -> query($queryInsert);
            (mysqli_affected_rows($databaseConnection))?$tester=true:$testerfalse;
        }else {
